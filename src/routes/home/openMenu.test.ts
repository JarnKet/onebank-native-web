...
describe('dispatch', () => {
  it('routes a page this app owns instead of framing it', async () => {
    ...
    await settle()
    expect(hash()).toContain('/transaction')
    expect(get(popups)).toHaveLength(0)
  })

  it('opens a b1hybrid page as an overlay instead of routing', async () => {
    expect(openMenu('STATEMENT')).toBe(true)
    await settle()
    expect(get(popups)).toHaveLength(1)
    expect(get(popups)[0].src).toContain('STATEMENT.html')
    expect(hash()).toBe('#/')
  })

  it('never routes a b1hybrid page, whatever the registry says', () => {
    for (const key of Object.keys(menus)) {
      const page = `${pageNameFor(key)}.html`
      if (!BCELONE_PAGES.includes(page)) continue
      popups.set([])
      window.location.hash = '#/'
      openMenu(key)
