...
import { navigateToPage } from '../../lib/utils/navigation'
...
export function openMenu(menuKey: string): boolean {
  ...
  const page = `${pageNameFor(menuKey)}.html`
  // A native screen reads the group from the stores; the legacy bundle (every
  // account, JSON-encoded) would only bloat its URL. It keeps the tile's own
  // registry params, which say *which* provider or service it is.
  if (isRoutedPage(page)) navigateToPage(page, entry.params ?? undefined)
  else showPopup(page, menuParams(entry.params))
  return true
}
