...
import {language, onebankPath, payloadPath} from '../../stores/config';
...
export const lang = parseInt(urlParams['lang'] as string ?? '0');
...
export function getProfileImageUrl(profileType: number | undefined, profileId: string, faceId: string, prefix: '' | 't.' | 'm.'): string {
    // `String(...)` rather than `profileType.toString()`: the type says this is
    // always present, but it arrives from the core, and one member record
    // missing it used to throw from inside the sidebar and take the page down.
    // An unknown type falls through to the placeholder, which is the point of
    // the default branch.
...
function buildPopupUrl(pagename: string, paramArray?: Record<string, any> | string, from?: string): string {
    ...
    const languageValue = get(language);
...
export function managePopup(options: PopupOptions): void {
    ...
    const menu = sidebarMenuItems.find(menu => menu.popupName === pagename.split(".")[0]);
    if (menu) {
        currentSidebarMenu.set(menu.id);
    }
}
