...
describe('navigateToPage', () => {
  it('routes a page this app owns and reports it handled it', async () => {
    ...
    expect(await settle()).toBe('#/transaction')
  })
  ...
  it('declines every page on the b1hybrid allowlist', async () => {
...
describe('the routed set and the b1hybrid set never overlap', () => {
  it('holds for every b1hybrid page', async () => {
    const overlap = BCELONE_PAGES.filter((page) => isRoutedPage(page))
