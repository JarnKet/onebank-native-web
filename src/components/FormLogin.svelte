<script lang="ts">
	...
	import {
		language, onebankPath, payloadPath,
	} from '../stores/config'
	import {onebankPath as resolveOnebankPath, payloadPath as resolvePayloadPath, serviceUrl} from '../lib/overrides';
	...
	async function handleSubmit(): Promise<void> {
		...
		if (devOverrides) {
			const before = serviceUrl();
			localStorage.coreip = coreip.trim();
			localStorage.onebankui = onebankUI.trim();
			localStorage.useproduction = isUseProduction;
			$onebankPath = resolveOnebankPath();
			$payloadPath = resolvePayloadPath();
			// A session is negotiated with one core; talking to another with it
			// would fail the handshake. Start fresh when the target moved.
			if (serviceUrl() !== before) Connector.clearSession();
		}
...
<form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }} class="space-y-5 p-6">
	<div class="space-y-1">
		...
		<div class="relative rounded-ob-sm shadow-sm">
			...
			<input
					...
					id="username"
					bind:value={username}
					class="block w-full rounded-ob-sm border-gray-300 pl-10 focus:border-onebank-red focus:ring-onebank-red text-sm"
	...
	<div class="space-y-1">
		...
		<div class="relative rounded-ob-sm shadow-sm">
			...
			<input
					...
					class={`block w-full rounded-ob-sm border-gray-300 pl-10 pr-10 focus:border-onebank-red focus:ring-onebank-red text-sm ${isPasswordSaved ? 'bg-green-50' : ''}`}
		...
		<div class="space-y-1">
			...
			<div class="relative rounded-ob-sm shadow-sm">
				...
				<input
						...
						id="coreip"
						bind:value={coreip}
						class={`block w-full rounded-ob-sm border-gray-300 pl-10 pr-10 focus:border-onebank-red focus:ring-onebank-red text-sm disabled:opacity-50 ${isUseProduction ? 'bg-gray-50' : ''}`}
	...
	{/if}
	{#if error}
		<div class="rounded-ob-sm bg-red-50 p-3 flex items-start">
